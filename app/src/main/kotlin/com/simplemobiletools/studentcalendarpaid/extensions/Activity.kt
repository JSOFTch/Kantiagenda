...
import com.simplemobiletools.studentcalendarpaid.dialogs.CustomEventRepeatIntervalDialog
...
fun BaseSimpleActivity.shareEvents(ids: List<Int>) {
    ...
    IcsExporter().exportEvents(this, file, events, false) {
        if (it == IcsExporter.ExportResult.EXPORT_OK) {
            sharePathIntent(file.absolutePath, com.simplemobiletools.studentcalendarpaid.BuildConfig.APPLICATION_ID)
        }
    }
}
...
fun Activity.showEventRepeatIntervalDialog(curSeconds: Int, callback: (minutes: Int) -> Unit) {
    ...
    items.add(RadioItem(-1, getString(com.simplemobiletools.studentcalendarpaid.R.string.custom)))
