...
class RepeatRuleWeeklyDialog(val activity: Activity, val curRepeatRule: Int, val callback: (repeatRule: Int) -> Unit) {
    ...
    val view = activity.layoutInflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.dialog_vertical_linear_layout, null)

    init {
        val days = activity.resources.getStringArray(com.simplemobiletools.studentcalendarpaid.R.array.week_days)
        val checkboxes = ArrayList<MyAppCompatCheckbox>(7)
        for (i in 0..6) {
            val pow = Math.pow(2.0, i.toDouble()).toInt()
            (activity.layoutInflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.my_checkbox, null) as MyAppCompatCheckbox).apply {
        ...
        dialog = AlertDialog.Builder(activity)
                .setPositiveButton(com.simplemobiletools.studentcalendarpaid.R.string.ok, { dialog, which -> callback(getRepeatRuleSum()) })
