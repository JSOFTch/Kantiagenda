...
import com.simplemobiletools.studentcalendarpaid.helpers.getNowSeconds
...
class RepeatLimitTypePickerDialog(val activity: Activity, var repeatLimit: Int, val startTS: Int, val callback: (repeatLimit: Int) -> Unit) {
    ...
    init {
        view = activity.layoutInflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.dialog_repeat_limit_type_picker, null).apply {
            repeat_type_date.setOnClickListener { showRepetitionLimitDialog() }
            repeat_type_count.setOnClickListener { dialog_radio_view.check(com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_x_times) }
        ...
        dialog = AlertDialog.Builder(activity)
                .setPositiveButton(com.simplemobiletools.studentcalendarpaid.R.string.ok) { dialogInterface, i -> confirmRepetition() }
    ...
    private fun getCheckedItem() = when {
        repeatLimit > 0 -> com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_till_date
        repeatLimit < 0 -> {
            view.repeat_type_count.setText((-repeatLimit).toString())
            com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_x_times
        }
        else -> com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_forever
    }
    ...
    private fun confirmRepetition() {
        when (view.dialog_radio_view.checkedRadioButtonId) {
            com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_till_date -> callback(repeatLimit)
            com.simplemobiletools.studentcalendarpaid.R.id.repeat_type_forever -> callback(0)
            else -> {
