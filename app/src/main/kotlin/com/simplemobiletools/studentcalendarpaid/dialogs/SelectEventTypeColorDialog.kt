...
class SelectEventTypeColorDialog(val activity: Activity, val eventType: EventType, val callback: (color: Int) -> Unit) {
    ...
    init {
        val view = activity.layoutInflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.dialog_select_event_type_color, null) as ViewGroup
    ...
    private fun addRadioButton(colorKey: Int, color: Int) {
        ...
        (view.dialog_radio_button as RadioButton).apply {
            text = if (color == 0) activity.getString(com.simplemobiletools.studentcalendarpaid.R.string.transparent) else String.format("#%06X", 0xFFFFFF and color)
