...
import com.simplemobiletools.studentcalendarpaid.interfaces.WeekFragmentListener
...
import com.simplemobiletools.studentcalendarpaid.views.MyScrollView
...
class WeekFragment : Fragment(), WeeklyCalendar {
    ...
    private var primaryColor = bgColor
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mRowHeight = mRes.getDimension(com.simplemobiletools.studentcalendarpaid.R.dimen.weekly_view_row_height)
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        mView = inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.fragment_week, container, false)
    ...
    private fun setupDayLabels() {
        ...
        for (i in 0..6) {
            ...
            val dayLetters = mRes.getStringArray(com.simplemobiletools.studentcalendarpaid.R.array.week_day_letters).toMutableList() as ArrayList<String>
        ...
        when (event.action) {
            ...
            MotionEvent.ACTION_UP -> {
                if (System.currentTimeMillis() - clickStartTime < CLICK_DURATION_THRESHOLD) {
                    ...
                    selectedGrid = (inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.week_grid_item, null, false) as ImageView).apply {
                        view.addView(this)
    ...
    private fun addEvents() {
        ...
        val fullHeight = mRes.getDimension(com.simplemobiletools.studentcalendarpaid.R.dimen.weekly_view_events_height)
        val minuteHeight = fullHeight / (24 * 60)
        val minimalHeight = mRes.getDimension(com.simplemobiletools.studentcalendarpaid.R.dimen.weekly_view_minimal_event_height).toInt()
        ...
        for (event in sorted) {
            if (event.getIsAllDay() || Formatter.getDayCodeFromTS(event.startTS) != Formatter.getDayCodeFromTS(event.endTS)) {
                ...
                (inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.week_event_marker, null, false) as TextView).apply {
    ...
    private fun addNewLine() {
        val allDaysLine = inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.all_day_events_holder_line, null, false) as RelativeLayout
    ...
    private fun addCurrentTimeIndicator(minuteHeight: Float) {
        if (todayColumnIndex != -1) {
            ...
            (inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.week_now_marker, null, false) as ImageView).apply {
                applyColorFilter(primaryColor)
                mView.week_events_holder.addView(this, 0)
                val extraWidth = (todayColumn.width * 0.3).toInt()
                val markerHeight = mRes.getDimension(com.simplemobiletools.studentcalendarpaid.R.dimen.weekly_view_now_height).toInt()
                (layoutParams as RelativeLayout.LayoutParams).apply {
    ...
    private fun addAllDayEvent(event: Event) {
        (inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.week_all_day_event_marker, null, false) as TextView).apply {
            var backgroundColor = eventTypeColors.get(event.eventType, primaryColor)
            var textColor = backgroundColor.getContrastColor()
            if (dimPastEvents && event.isPastEvent) {
                backgroundColor = backgroundColor.adjustAlpha(LOW_ALPHA)
