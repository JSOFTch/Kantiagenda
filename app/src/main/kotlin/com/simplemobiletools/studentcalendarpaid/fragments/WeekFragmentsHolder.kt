...
import com.simplemobiletools.studentcalendarpaid.adapters.MyWeekPagerAdapter
...
import com.simplemobiletools.studentcalendarpaid.helpers.WEEK_START_DATE_TIME
...
class WeekFragmentsHolder : MyFragmentHolder(), WeekFragmentListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        weekHolder = inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.fragment_week_holder, container, false) as ViewGroup
    ...
    private fun setupFragment() {
        ...
        for (i in 1..23) {
            ...
            (layoutInflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.weekly_view_hour_textview, null, false) as TextView).apply {
                text = formattedHours
    ...
    private fun setupWeeklyActionbarTitle(timestamp: Int) {
        ...
        if (startDateTime.monthOfYear == endDateTime.monthOfYear) {
            ...
            (activity as com.simplemobiletools.studentcalendarpaid.activities.MainActivity).updateActionBarTitle(newTitle)
        } else {
            val endMonthName = Formatter.getMonthName(context!!, endDateTime.monthOfYear)
            (activity as com.simplemobiletools.studentcalendarpaid.activities.MainActivity).updateActionBarTitle("$startMonthName - $endMonthName")
        }
        (activity as com.simplemobiletools.studentcalendarpaid.activities.MainActivity).updateActionBarSubtitle("${getString(com.simplemobiletools.studentcalendarpaid.R.string.week)} ${startDateTime.plusDays(3).weekOfWeekyear}")
    }
