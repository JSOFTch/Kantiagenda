package com.simplemobiletools.studentcalendarpaid.views
...
class MonthViewWrapper(context: Context, attrs: AttributeSet, defStyle: Int) : FrameLayout(context, attrs, defStyle) {
    ...
    init {
        val normalTextSize = resources.getDimensionPixelSize(com.simplemobiletools.studentcalendarpaid.R.dimen.normal_text_size).toFloat()
        weekDaysLetterHeight = 2 * normalTextSize.toInt()

        inflater = LayoutInflater.from(context)
        monthView = inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.month_view, this).month_view
    ...
    private fun setupHorizontalOffset() {
        horizontalOffset = if (context.config.showWeekNumbers) resources.getDimensionPixelSize(com.simplemobiletools.studentcalendarpaid.R.dimen.smaller_text_size) * 2 else 0
    }
    ...
    private fun addViews() {
        removeAllViews()
        monthView = inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.month_view, this).month_view as MonthView
        wereViewsAdded = true
        var curId = 0
        for (y in 0..5) {
            for (x in 0..6) {
    ...
    private fun addViewBackground(xPos: Float, yPos: Float, day: DayMonthly) {
        inflater.inflate(com.simplemobiletools.studentcalendarpaid.R.layout.month_view_background, this, false).apply {
